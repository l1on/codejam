...
class JudgeSingleCase(object):
  ...
  def __init__(self, io, initial_arr, caseNum):
    self.io = io
  ...
  def Judge(self):
    ...
    for i in range(MAX_QUERIES + 1):
      ...
      if i % 10 == 0:
        ...
        #print('caseNum: {}, queries: {} Entering fluctuation zone...'.format(self.caseNum, i+1), file=sys.stderr)
        ...
        #print('caseNum: {}, queries: {} Exit fluctuation zone with bits {}'.format(self.caseNum, i+1, self.arr), file=sys.stderr)
...
def JudgeAllCases(test_number, io):
  ...
  for case_number in range(NUM_CASES):
    single_case = JudgeSingleCase(io, inputs[case_number], case_number + 1)
